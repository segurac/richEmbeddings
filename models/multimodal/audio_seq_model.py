...
class AudioFB_sequence_model(nn.Module):
	...
	nn.ReLU(),
        ...
        #self.classifier = nn.Linear(nhid  * self.num_directions, self.audio_embedding_size)

        self.final_feature_size = 128
        self.rnn_features = nn.Linear(nhid * self.num_directions, self.final_feature_size)
        self.classifier = nn.Linear(self.final_feature_size,self.audio_embedding_size)
